...
**分段穷举策略（必须执行）**：
  1. 先梳理人物的主要旅行阶段（按年份/时期分段，如早年/中年/晚年，或每次独立出行）
  2. 对每个阶段**独立搜索**，不允许以"全生涯统一搜索"代替分段
  3. 每个阶段搜索完成后，列出本阶段已提取的地点清单，再进入下一阶段
  4. 所有阶段搜索完毕后，将各阶段节点汇总为完整列表，交给 Agent 3 验证

**省级节点使用规则**：
  - 禁止以省级/地区级模糊节点（如「云南」「广西」「湖南」）代替具体城市/景点
  - 仅当穷尽搜索（年谱、正史、游记原文、地方志）后确实找不到具体地点时，才允许使用省级节点作为降级方案
  - 降级使用时必须：
      · place_raw 保留原始记载（如「入滇」「经湖广」）
      · place_modern 填省级中心城市坐标作近似
      · confidence 标为 "inferred" 或 "single_source"
      · description 注明「仅知进入该省/地区，具体行程不详」
...
**遗漏检测（必须执行）**：
  1. 对照人物年谱/传记/大事记，逐次出行检查 Agent 2 的节点列表
     - 年谱中有记载的出行，在 waypoints 中有对应节点 → 正常
     - 年谱中有记载但 waypoints 中无对应节点 → 标记为疑似遗漏，补充搜索
  2. 检查时间跨度：若相邻两节点之间时间跨度超过半年，且行程空白，
     须在 CHECKPOINT 中主动说明原因（居家、养病、史料缺失等），
     不能无声略过
  3. 遗漏检测结果在 CHECKPOINT 中单独列出「疑似遗漏地点」一栏供用户确认
