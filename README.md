...
├── output/                           # 生成的地图和中间文件（.gitignore 已排除）
└── examples/
    ├── mode-b-xu-xiake/              # 示例：徐霞客人生游历（Mode B）
    │   ├── 徐霞客游历路线.html        # 地图文件，浏览器直接打开
    │   └── 徐霞客_waypoints_final.json
    └── mode-a-central-asia/          # 示例：《中亚行纪》埃丽卡·法特兰（Mode A）
        ├── 中亚行纪：法特兰游记路线.html
        └── 中亚行纪_waypoints_v3_final.json
```
...
基于游记年谱与传记文献，检索并提取徐霞客 1607–1641 年间的完整游历路线，共 **52 个地理节点**（主要节点 48 处 + 途经点 4 处），**52/52 坐标全部解析成功**。

路线从江阴故里出发，经山东（泰山、峄山）、浙江（天台山、雁荡山、普陀山）、安徽（黄山、齐云山）、福建、江西（庐山）、华中五岳（嵩山、华山、武当山、五台山、恒山、盘山），最终完成大西南四年壮游，深入云南腾冲（最西端，98°E），1641 年返回江阴病逝。

地图文件：`examples/mode-b-xu-xiake/徐霞客游历路线.html`

详见：[examples/mode-b-xu-xiake/](examples/mode-b-xu-xiake/)

### 示例 2：《中亚行纪》埃丽卡·法特兰（Mode A）

从 epub 电子书中提取挪威旅行作家埃丽卡·法特兰（Erika Fatland）的中亚五国旅行路线，共 **36 个地理节点**，**36/36 坐标全部解析成功**。

路线覆盖土库曼斯坦（达瓦扎"地狱之门"）→ 哈萨克斯坦（咸海干涸现场、苏联核试验区）→ 塔吉克斯坦（帕米尔高原霍罗格）→ 吉尔吉斯斯坦（传统养鹰人、族群冲突旧址）→ 乌兹别克斯坦（希瓦古城、撒马尔罕）。

地图文件：`examples/mode-a-central-asia/中亚行纪：法特兰游记路线.html`
